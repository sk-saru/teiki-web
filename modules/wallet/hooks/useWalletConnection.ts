...
import { WalletAuthHeaderInfo, WalletStatus } from "../types";
...
export function useWalletConnection({ provider, network }: Params): Results {
  ...
  const authenticate = async (lucid: Lucid): Promise<WalletAuthHeaderInfo> => {
  ...
  const connectWallet = async (walletName: string): Promise<WalletStatus> => {
    ...
    try {
      ...
      const headerInfo = await authenticate(lucid);
      setWalletStatus({ status: "connected", info, lucid, headerInfo });
      return { status: "connected", info, lucid, headerInfo };
    } catch (error) {
